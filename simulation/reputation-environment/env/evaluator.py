...
class EnvironmentRecorder():
    ...
    def on_env_step(self, env):
        ...
        for agent, action in env.actions.items():
            ...
            self.author_efforts[agent] += list(
                paper_info['effort'][finished][np.nonzero(paper_info['effort'][finished])]
            )
            if action["contribute"] > 0:
                self.actions_taken[agent]["contribute"] = self.actions_taken[agent].get("contribute", 0) + 1

            n_coauthors = int(action["start_with_coauthors"]) - 1
            if n_coauthors>=0:
                self.actions_taken[agent]["start_with_coauthors"] = self.actions_taken[agent].get(
                    "start_with_coauthors", {}
                )
            
                self.actions_taken[agent]["start_with_coauthors"][n_coauthors] = self.actions_taken[
                    agent
                ][
                    "start_with_coauthors"
                ].get(n_coauthors, 0) + 1
            ...
            if action["submit"]["id"] > 0:
                self.actions_taken[agent]["submit"] = self.actions_taken[agent].get("submit", 0) + 1
                self.actions_taken[agent]["submit_to"] = self.actions_taken[agent].get("submit_to", {})
                conf = int(action["submit"]["conference"])
                self.actions_taken[agent]["submit_to"][conf] = self.actions_taken[agent]["submit_to"].get(
                    conf, 0
                ) + 1
    ...
    def get_avg_coauthors(self, agent=None):
        if agent is None:
            ...
            avg = np.sum([
                np.sum([
                    constellation * val
                      for constellation, val in actions["start_with_coauthors"].items()
                ]) for actions in self.actions_taken.values()
            ]) / nr_papers
        else:
            ...
            avg = np.sum([
                constellation * val
                  for constellation, val in self.actions_taken[agent]["start_with_coauthors"].items()
            ]) / nr_papers
        return avg
    ...
    def get_percentage_of_strategy(self, strategy=None):
        if strategy is None:
            ...
            percentage_strats = {strat: count/all_strats for strat, count in count_strats.items()}
            return percentage_strats

    
    def report(self):
        # print(self.record_env.network)
        print(f"simulation with {self.record_env.n_authors} authors went on for {self.nr_steps} steps.")
        ...
        print(f"percentage of strategies used: {json.dumps(self.get_percentage_of_strategy(), indent=2)}")
        print(f"mean reputation increase {np.mean(self.record_env.reputations-self.record_env.initial_reputation)}")
